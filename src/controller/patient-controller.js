...
async function addPatient(req, res, next) {
    try {
        ...
        res.status(500).send(new ApiResponse(500, "Error while adding patient.", null, error.message));
    }
}
...
async function editPatient(req, res, next) {
    try {
        console.log("Controller received request to edit patient with phone number:", req.params.phone);
        const result = await PatientService.editPatient(req.params.phone, req.body, req.user);
...
async function listPatient(req, res, next) {
    try {
        ...
        const patients = await PatientService.listPatient();
        console.log("List Patient Result:", patients);
        res.status(200).send(new ApiResponse(200, "Patients fetched successfully.", null, patients));
    } catch (error) {
        console.error("Error while fetching patients:", error.message);
        res.status(500).send(new ApiResponse(500, "Error while fetching patients.", null, error.message));
    }
}

...
async function addAppointment(req, res, next) {
    try {
        console.log("Controller received request to add appointment:", req.body.phone);
        ...
        return res.status(500).send(new ApiResponse(500, "Error while adding appointment.", null, error.message));
    }
}
...
async function editAppointment(req, res, next) {
    try {
        ...
        const patient_phone = req.params.patient_phone || req.query.patient_phone || req.body.patient_phone;

        if (!patient_phone) {            
            console.log(" Error: Phone number is missing or invalid");
            return res.status(400).json(new ApiResponse(400, "Phone number is required", null, "Phone number is missing or invalid"));
        }
        ...
        if (!formattedPhone) {
            ...
            return res.status(400).json(new ApiResponse(400, "Phone number format is invalid", null, "Invalid phone number format"));
        }

        console.log("🔹 Calling AppointmentService.editAppointment for phone number:", formattedPhone);

        // Assuming the rest of your code is correct
        const result = await AppointmentService.editAppointment(formattedPhone, req.body, req.user);
        ...
        return res.status(500).json(new ApiResponse(500, "Error while editing appointment.", null, error.message));
    }
}
...
async function listAppointments(req, res, next) {
    try {
        ...
        if (!from || !to) {
            ...
            return res.status(400).send(new ApiResponse(400, "Missing 'from' or 'to' date in query params.", null, null));
        }

        // Call service with correctly extracted params
        const appointments = await AppointmentService.listAppointments({ from, to });
...
async function confirmAppointment(req, res) {
    try {
        ...
        console.log("Controller received request to confirm appointment:", appointment_id);
        if (!appointment_id) return res.status(400).send(new ApiResponse(400, "Appointment ID is required.", null, null));
        const result = await AppointmentService.confirmAppointment(appointment_id);
        return res.status(result.statusCode).send(result);
    } catch (error) {
        console.error("Error while confirming appointment:", error.message);
        return res.status(500).send(new ApiResponse(500, "Error while confirming appointment.", null, error.message));
    }
}
...
async function doctorDropdown(req, res, next) {
    try {
        ...
        const data = await AppointmentService.doctorDropdown();
        return res.status(data.length ? 200 : 404).send(new ApiResponse(200, "Doctor dropdown fetched successfully.", null, data));
    } catch (error) {
...
async function consultationDropdown(req, res, next) {
    try {
        ...
        return res.status(data.length ? 200 : 404).send(new ApiResponse(200, "Consultation dropdown fetched successfully.", null, data));
    } catch (error) {
        console.error("Error while fetching consultation dropdown:", error.message);
        return res.status(500).send(new ApiResponse(500, "Error while fetching consultation dropdown.", null, error.message));
    }
}
...
async function fdeDropdown(req, res, next) {
    try {
        ...
        const data = await AppointmentService.fdeDropdown();
        return res.status(data.length ? 200 : 404).send(new ApiResponse(200, "FDE dropdown fetched successfully.", null, data));
    } catch (error) {
...
async function departmentDropdown(req, res, next) {
    try {
        ...
        return res.status(data.length ? 200 : 404).send(new ApiResponse(200, "Department dropdown fetched successfully.", null, data));
    } catch (error) {
        console.error("Error while fetching department dropdown:", error.message);
        return res.status(500).send(new ApiResponse(500, "Error while fetching department dropdown.", null, error.message));
    }
}
...
async function treatmentDropdown(req, res, next) {
    try {
        ...
        return res.status(data.length ? 200 : 404).send(new ApiResponse(200, "treatment dropdown fetched successfully.", null, data));
    } catch (error) {
        console.error("Error while fetching treatment dropdown:", error.message);
        return res.status(500).send(new ApiResponse(500, "Error while fetching treatment dropdown.", null, error.message));
    }
}
...
async function updateHistoryChk(req, res) {
    try {
        const { appointment_id } = req.params;
        console.log("Controller received request to update historyChk:", appointment_id);
        if (!appointment_id) return res.status(400).send(new ApiResponse(400, "Appointment ID is required.", null, null));
        const result = await AppointmentService.updateHistoryChk(appointment_id);
        return res.status(result.statusCode).send(result);
    } catch (error) {
        console.error("Error while updating historyChk:", error.message);
        return res.status(500).send(new ApiResponse(500, "Error while updating historyChk.", null, error.message));
    }
}
...
async function updateExecutionChk(req, res) {
    try {
        const { appointment_id } = req.params;
        console.log("Controller received request to update executionChk:", appointment_id);
        if (!appointment_id) return res.status(400).send(new ApiResponse(400, "Appointment ID is required.", null, null));
        const result = await AppointmentService.updateExecutionChk(appointment_id);
        return res.status(result.statusCode).send(result);
    } catch (error) {
        console.error("Error while updating executionChk:", error.message);
        return res.status(500).send(new ApiResponse(500, "Error while updating executionChk.", null, error.message));
    }
}
...
async function updateExecutionChkToFour(req, res) {
    try {
        const { appointment_id } = req.params;
        console.log("Controller received request to update executionChk to 4:", appointment_id);
        if (!appointment_id) return res.status(400).send(new ApiResponse(400, "Appointment ID is required.", null, null));
        const result = await AppointmentService.updateExecutionChkToFour(appointment_id);
        return res.status(result.statusCode).send(result);
    } catch (error) {
        console.error("Error while updating executionChk to 4:", error.message);
        return res.status(500).send(new ApiResponse(500, "Error while updating executionChk to 4.", null, error.message));
    }
}
...
async function updateConsultationChk(req, res) {
    try {
        const { appointment_id } = req.params;
        console.log("Controller received request to update consultationChk:", appointment_id);
        if (!appointment_id) return res.status(400).send(new ApiResponse(400, "Appointment ID is required.", null, null));
        const result = await AppointmentService.updateConsultationChk(appointment_id);
        return res.status(result.statusCode).send(result);
    } catch (error) {
        console.error("Error while updating consultationChk:", error.message);
        return res.status(500).send(new ApiResponse(500, "Error while updating consultationChk.", null, error.message));
    }
}
...
async function saveReceipt(req, res, next) {
    try {
        ...
        return res.status(500).send(new ApiResponse(500, "Error while saving Receipt.", null, error.message));
    }
}
...
async function getPatientByMobile(req, res, next) {
  try {
    ...
    if (!patient_phone) {
      return res.status(400).send(new ApiResponse(400, "Mobile number is required.", null, null));
    }

    console.log("Controller received request to fetch patient by mobile number:", patient_phone);
    ...
    console.error("Error while fetching patient by mobile number:", error.message);
    res.status(500).send(new ApiResponse(500, "Error while fetching patient details.", null, error.message));
  }
}
async function listReceipt(req, res) {
    try {
        ...
        res.status(500).send(new ApiResponse(500, "Error while fetching receipts.", null, error.message));
    }
}
...
async function deleteAppointment(req, res) {
    try {
        ...
        console.log("Controller received request to delete appointment:", appointment_id);

        if (!appointment_id) {
            return res.status(400).send(new ApiResponse(400, "Appointment ID is required.", null, null));
        }
        ...
        if (result) {
            return res.status(200).send(new ApiResponse(200, "Appointment marked as deleted successfully.", null, null));
        } else {
            return res.status(404).send(new ApiResponse(404, "Appointment not found.", null, null));
        }
    } catch (error) {
        console.error("Error while deleting appointment:", error.message);
        return res.status(500).send(new ApiResponse(500, "Error while deleting appointment.", null, error.message));
    }
}
